...
void hill_climbing(struct point** points,
    ...
    FILE *f_progression = fopen("./output/out-hc-progression.txt", "a");
    if (NULL == f_progression) {
        perror("./output/out-hc-progression.txt");
        exit(2);
    }
    ...
    while (num_evals < MAX_ITER) {
        ...
        for (int i=1; i<num_points; i++) {
            ...
            if (lt_gt(new_dist, best_swap_dist)) {
        ...
        if (best_swap_pos == 0) {
            ...
            if (current_dist > hilltop_dist) {
                hilltop_dist = current_dist;
                copy_path(&current_path, &hilltop_path, num_points);
            }
